...
object ConstraintFactory {
   ...
   */
  def makeFactors[T](cc: ComponentCollection, elem: Element[T], upper: Boolean): List[Factor[(Double,Double)]] =
  ...
  private def makeConditionFactor[T](cc: ComponentCollection, elem: Element[T], contingentCondition: (T => Boolean, Element.Contingency), upper: Boolean): Factor[(Double,Double)] =
    makeConstraintFactor(cc, elem, (ProbConstraintType((t: T) => if (contingentCondition._1(t)) 1; else 0.0), contingentCondition._2), upper)

  private def makeConstraintFactor[T](cc: ComponentCollection, elem: Element[T], contingentConstraint: (T => Double, Element.Contingency), upper: Boolean): Factor[(Double,Double)] = {
  ...
  private def makeUncontingentConstraintFactor[T](cc: ComponentCollection, elem: Element[T], constraint: T => Double, upper: Boolean): Factor[(Double, Double)] = {
    val elemVar = Factory.getVariable(cc, elem)
    val factor = new ConstraintFactor[(Double,Double)](List(), List(elemVar))
    for { (elemVal, index) <- elemVar.range.zipWithIndex } {
      val entry =
        if (elemVal.isRegular) {
          math.exp(constraint(elemVal.value))
        } else if (upper) {
          SumProductDualSemiring().one
        } else {
          SumProductDualSemiring().zero
        }
      factor.set(List(index), (entry.asInstanceOf[Double], 0.0))
    }
    factor
  }

  private def makeContingentConstraintFactor[T](cc: ComponentCollection, elem: Element[T], constraint: T => Double,
      firstConting: Element.ElemVal[_], restContinges: Element.Contingency, upper: Boolean): Factor[(Double,Double)] = {
  ...
  private def extendConstraintFactor(cc: ComponentCollection, restFactor: Factor[(Double,Double)], firstConting: Element.ElemVal[_], upper: Boolean): Factor[(Double,Double)] = {
    ...
    val resultFactor = new ConstraintFactor[(Double,Double)](firstVar :: restFactor.parents, restFactor.output)
    for { restIndices <- restFactor.getIndices } {
      ...
      for { (firstValue, firstIndex) <- firstValues.zipWithIndex } {
        if (firstValue.isRegular) {
          val resultEntry = if (firstIndex == matchingIndex) restEntry; else restFactor.semiring.one
          ...
          val entry = if (upper) restFactor.semiring.one else restEntry
