...
object Factory {
   ...
   */
  def replaceVariable[T](factor: Factor[(Double,Double)], oldVariable: Variable[T], newVariable: Variable[T]): Factor[(Double,Double)] = {
    ...
    if (!factor.variables.contains(oldVariable)) factor
      ...
      val newFactor =
        factor match {
          case s: SparseFactor[(Double,Double)] => new SparseFactor[(Double,Double)](newVariables, List())
          case b: DenseFactor[(Double,Double)] => new DenseFactor[(Double,Double)](newVariables, List())
        }
  ...
  def makeTupleVarAndFactor(cc: ComponentCollection, chain: Option[Chain[_, _]], inputs: Variable[_]*): (Variable[List[Extended[_]]], Factor[(Double,Double)]) = {
    ...
    val tupleFactor = new SparseFactor[(Double,Double)](inputList, List(tupleVar))
    for { pair <- tupleVar.range.zipWithIndex } {
      ...
      tupleFactor.set(inputIndices ::: List(tupleIndex), (1.0, if (pair._1.isDual) 1.0 else 0.0 ))
    }
   ...
   */
  def defaultFactor[T: TypeTag](parents: List[Variable[_]], children: List[Variable[_]], _semiring: Semiring[T] = SumProductDualSemiring().asInstanceOf[Semiring[T]]) =
  ...
  private def makeFactors[T](cc: ComponentCollection, const: Constant[T]): List[Factor[(Double,Double)]] = {
    //println("Making Factor For: " + const)
    val factor = new DenseFactor[(Double,Double)](List(), List(getVariable(cc, const)))
  ...
  def makeConditionalSelector[T, U](pairVar: Variable[List[Extended[_]]], parentXVal: Extended[T], outcomeVar: Variable[U], choices: Set[U])(implicit mapper: PointMapper[U]): Factor[(Double,Double)] = {
    val factor = new ConditionalSelector[(Double,Double)](List(pairVar), List(outcomeVar))
    for {
      ...
      /*if (outcomeVar.isDual) {
        println("This is a huge hack and probably broken - dscof - outcome: " + outcomeVar + " - pair var:  "+ pairVar)
        if (pairIndex % 2 == 0) {
          1.0
        } else { -1.0 }
      }  else {
  ...
  private def makeFactors[T](cc: ComponentCollection, atomic: Atomic[T]): List[Factor[(Double,Double)]] = {
  ...
  def convertToDual(f: Factor[Double], elem: Element[_]): Factor[(Double, Double)] = {

    if (!elem.isDual) {
      f.mapTo[(Double, Double)]((d: Double) => (d, 0.0), f.semiring.asInstanceOf[Semiring[(Double, Double)]])
    } else {
      //      if (f.variables.length > 1 && f.variables.head.range.size > 1) throw new IllegalUtilityNodeException
      val newF = f.mapTo[(Double, Double)]((d: Double) => (d, 1.0 ), f.semiring.asInstanceOf[Semiring[(Double, Double)]])
      newF
    }

  }
  def convertToZeroDual(f: Factor[Double]): Factor[(Double, Double)] = {

      val newF = f.mapTo[(Double, Double)]((d: Double) => (d, 0.0 ), f.semiring.asInstanceOf[Semiring[(Double, Double)]])
    newF
  ...
  def concreteFactors[T](cc: ComponentCollection, elem: Element[T], parameterized: Boolean): List[Factor[(Double, Double)]] = {
    ...
    val f = elem match {
      case pBin: ParameterizedBinomialFixedNumTrials => return DistributionFactory.makeFactors(cc, pBin, parameterized) // ibid
      case flip: ParameterizedFlip => return DistributionFactory.makeFactors(cc, flip, parameterized)
      case f: AtomicFlip => return DistributionFactory.makeFactors(cc, f)  // ibid
      case f: CompoundFlip => return DistributionFactory.makeFactors(cc, f)
      case s: AtomicSelect[_] => return SelectFactory.makeFactors(cc, s)
      case constant: Constant[_] => return makeFactors(cc, constant) // ibid
      case c: Chain[_, _] => return ChainFactory.makeFactors(cc, c)
      case m: MakeArray[_] => return ComplexFactory.makeFactors(cc, m)
      case r: SingleValuedReferenceElement[_] => return ComplexFactory.makeFactors(cc, r)
      case r: MultiValuedReferenceElement[_] => return ComplexFactory.makeFactors(cc, r)
      case r: Aggregate[_, _] => return ComplexFactory.makeFactors(cc, r)
      case a: Atomic[_] => return makeFactors(cc, a)
      case d: AtomicDist[_] => return SelectFactory.makeFactors(cc, d)
      case d: CompoundDist[_] => return SelectFactory.makeFactors(cc, d)
      case s: CompoundSelect[_] => return SelectFactory.makeFactors(cc, s)
      case pSelect: ParameterizedSelect[_] => return SelectFactory.makeFactors(cc, pSelect, parameterized) // list factor double
      case _ => null
    }

    // convertToDual
    val toConvert = elem match {
      case parameter: DoubleParameter if parameterized => makeParameterFactors(cc, parameter) // ibid
      case array: ArrayParameter if parameterized => makeParameterFactors(cc, array) // ibid
    ...
    val converted = toConvert.map({ f => convertToDual(f, elem)})
    converted
  }
  ...
  private def makeAbstract[T](cc: ComponentCollection, elem: Element[T], abstraction: Abstraction[T]): List[Factor[(Double,Double)]] = {
    println("Making Abstract Factor For: " + elem)
    elem match {
      case apply: Apply1[_, _] => ApplyFactory.makeFactors(cc, apply)(abstraction.scheme).map(convertToDual(_, elem))
      case apply: Apply2[_, _, _] => ApplyFactory.makeFactors(cc, apply)(abstraction.scheme).map(convertToDual(_, elem))
      case apply: Apply3[_, _, _, _] => ApplyFactory.makeFactors(cc, apply)(abstraction.scheme).map(convertToDual(_, elem))
      ...
      case chain: Chain[_, _] => ChainFactory.makeFactors(cc, chain)(abstraction.scheme) //.map(convertToDual(_, elem))
  ...
  def makeFactors[T](cc: ComponentCollection, elem: Element[T], parameterized: Boolean): List[Factor[(Double,Double)]] = {
    println("Make Non Constraint Factors: " + elem)
    ...
    if (component.range.hasStar && component.range.regularValues.isEmpty) List()
      ...
      Abstraction.fromPragmas(elem.pragmas) match {
        case None => {
          println("concrete factors")
          concreteFactors(cc, elem, parameterized)
        }
        case Some(abstraction) => {
          println("abstract factors")
          makeAbstract(cc, elem, abstraction)
        }
      }
    }
  }
  ...
  def makeDependentFactor(cc: ComponentCollection, parentUniverse: Universe,
    ...
    val factor = new DenseFactor[(Double,Double)](variables, List())
    factor.fillByRule(rule _)
    factor
  }
   ...
   */
  def makeFactorsForElement[Value](elem: Element[_], upper: Boolean = false, parameterized: Boolean = false): List[Factor[(Double,Double)]] = {
    val variable = Variable(elem)
    val comp = Variable.cc(elem)
    println("Make Factors For Element: " + comp.element + ", " + " components: " + comp.problem.components)
    if (elem.intervention.isDefined) {
      println("Intervention Defined")
      val factor = new DenseFactor[(Double,Double)](List(), List(variable))
      val dual = 0.0
//      val dual = elem.isDual match {
//        case true =>   1.0
//        case _ => 0.0
//
//      }
      factor.set(List(0),(1.0,dual))
      List(factor)
    }
    else {
      comp match {
        ...
        case chainComp: ChainComponent[_, _] =>
          ...
          //println("Making Factor For Chain Variables: " + chainComp.actualSubproblemVariables + " which is part of " + chain)
          val chainMap = LazyValues(elem.universe).getMap(chain)
          println("Making Factor For Chain: " + chain + " which uses args " + chain.args)
          chainMap.foreach(f => {
            val subproblem = new NestedProblem(Variable.cc, f._2)
            println("SubProblem Target: " + subproblem.target + ", " + subproblem.target.isDual)
          ...
          println("Updated Subproblems: " + chainComp.subproblems)
        ...
        case applyComp: ApplyComponent[Value] =>
          println("Element is an ApplyComp")
      ...
      val c = constraint // .map( { f => f.mapTo[(Double, Double)]((d: Double) => (d, 0.0), f.semiring.asInstanceOf[Semiring[(Double, Double)]])})
      val nc = comp.nonConstraintFactors(parameterized)
      val ret = c ::: nc
      println("Returns: " + ret)
      ret
