...
//
//
//trait AutomaticDifferentiationVariableElimination extends VariableElimination[(Double, Double)] {
//  
...
//    if (debug) {
//      println("Elements (other than thisUniverseFactorsExceptDualities) appearing in factors and their ranges:")
...
//    
//    val chains = neededElements //.filter { p => p.isInstanceOf[Chain[_,_]] }
//    val nonChains = neededElements.filter { p => !p.isInstanceOf[Chain[_,_]] }
//    
//    val chainFactors = chains.map { x => Factory.makeFactorsForElement(x) }
//    val nonChainFactors = nonChains.map { x => Factory.makeFactorsForElement(x) }
//    val allfactors = neededElements.map { x => Factory.makeFactorsForElement(x) }
//    println("neededElements:")
//    neededElements.foreach { x => println(x) }
//    
//    val (factorsInNeededElements) = allfactors.filter(x => neededElements.contains(x))
//    val (factorsNotInNeededElements) = allfactors.filter(x => !neededElements.contains(x))
//    println("factorsInNeededElements")
//    factorsInNeededElements.foreach { x => println(x) }
//    println("factorsNotInNeededElements")
//    factorsNotInNeededElements.foreach { x => println(x) }
//
//    //val allfactorsToVariables = allfactors.filter { p => p. }
//    
////    val productChainFactors =  chainFactors reduceLeft (_.product(_))
////    val productNonChainFactors =  nonChainFactors reduceLeft (_.product(_))
//   
//   // resultFactor = productFactor.sumOver(variable)
//    
...
//        
//    val thisUniverseFactorsUtil = getDualNodes flatMap (Factory.makeFactorsForElement(_))
//    
//    println("FactorsExceptDual:")
//    thisUniverseFactorsExceptDual.foreach { x => println(x) }
//    
//    println("thisUniverseFactorsUtil:")
//    thisUniverseFactorsUtil.foreach { x => println(x) }
//    
...
//    val thisUniverseFactorsUtil_conv = thisUniverseFactorsUtil.map(s => convert(s, true)):::factorsWithDuals.map(s => convert(s, true))
//    val dependentUniverseFactors_conv = dependentUniverseFactors.map(s => convert(s, false))
//
//    val ret = dependentUniverseFactors_conv ::: thisUniverseFactorsExceptDual_conv ::: thisUniverseFactorsUtil_conv
//    val ret2 = ret.map({ x => 
//      println("DEBUG: " + x.toString())
//      if (x.toString().contains("Factor(32,23")) {
//        x.set(List(2), (1.0, -1.0))
//        x.set(List(3), (1.0, -1.0))
////        x.set(List(1), (0.0, 0.0))
////        x.set(List(3), (1.0, 0.0))
//         x
//      } else {
//        x
//      }
//      })
//      ret2
////        dependentUniverseFactors_conv ::: thisUniverseFactorsExceptDual_conv ::: thisUniverseFactorsUtil_conv
//  }
//
...
//  with AutomaticDifferentiationVariableElimination
//  {
