...
/**
  ...
  * Generic type U is either an Element or a Reference. T is the type of the element or reference.
  */
trait BaseDualProbQueryAlgorithm[U[_]]
  extends Algorithm {
  ...
  /**
    ...
    * ordered.
    */
  def computeDistribution[T](target: U[T]): Stream[((Double,Double), T)]

  /**
    * Return an estimate of the expectation of the function under the marginal probability distribution
    * of the target.
    */
  def computeExpectation[T](target: U[T], function: T => (Double,Double)): (Double,Double)

  /**
    * Return an estimate of the probability of the predicate under the marginal probability distribution
    * of the target.
    */
  def computeProbability[T](target: U[T], predicate: T => Boolean): (Double, Double) = {
    computeExpectation(target, (t: T) => if (predicate(t)) (1.0, 0.0); else (0.0, 0.0))
  }

  protected[algorithm] def computeProjection[T](target: U[T]): List[(T, (Double,Double))] = {
    projectDistribution(computeDistribution(target))
  }

  private def projectDistribution[T](distribution: Stream[((Double,Double), T)]): List[(T, (Double,Double))] = {
    (distribution map (_.swap)).toList
  }
  ...
  protected def doDistribution[T](target: U[T]): Stream[((Double,Double), T)]

  protected def doExpectation[T](target: U[T], function: T => (Double,Double)): (Double,Double)

  protected def doProbability[T](target: U[T], predicate: T => Boolean): (Double,Double)

  protected def doProjection[T](target: U[T]): List[(T, (Double,Double))] = {
    projectDistribution(doDistribution(target))
  }
  ...
  /**
    ...
    * Throws AlgorithmInactiveException if the algorithm is inactive.
    */
  def distribution[T](target: U[T]): Stream[((Double,Double), T)] = {
    check(target)
    doDistribution(target)
  }
  ...
  /**
    ...
    * Throws AlgorithmInactiveException if the algorithm is inactive.
    */
  def expectation[T](target: U[T], function: T => (Double,Double)): (Double,Double) = {
    check(target)
    doExpectation(target, function)
  }
  ...
  /**
    ...
    * Throws AlgorithmInactiveException if the algorithm is inactive.
    */
  def expectation[T](target: U[T])(function: T => (Double,Double), c: Any = DummyImplicit): (Double,Double) = {
    check(target)
    doExpectation(target, function)
  }
  ...
  /**
    * Return the mean of the probability density function for the given continuous element.
    */
  def mean(target: U[(Double,Double)]): (Double,Double) = {
    expectation(target, (d: (Double,Double)) => d)
  }

  /**
    * Return the variance of the probability density function for the given continuous element.
    */
  def variance(target: U[(Double,Double)]): (Double,Double) = {
    val m = mean(target)
    val s = SumProductDualSemiring()
    val ex2 = expectation(target, (d: (Double,Double)) => s.product(d, d))
    s.sum(ex2, s.product((-1.0,0.0), s.product(m,m)))
  }
  ...
  /**
    ...
    * Throws AlgorithmInactiveException if the algorithm is inactive.
    */
  def probability[T](target: U[T], predicate: T => Boolean): (Double,Double) = {
    check(target)
    doProbability(target, predicate)
  }
  ...
  /**
    ...
    * Throws AlgorithmInactiveException if the algorithm is inactive.
    */
  def probability[T](target: U[T])(predicate: T => Boolean, c: Any = DummyImplicit): (Double,Double) = {
    probability(target, predicate)
  }
  ...
  /**
    ...
    * Throws AlgorithmInactiveException if the algorithm is inactive.
    */
  def probability[T](target: U[T], value: T): (Double,Double) = {
    check(target)
    doProbability(target, (t: T) => t == value)
  }
}
...
/**
  * Algorithms that compute conditional probabilities of queries over elements in a universe.
  */
trait DualProbQueryAlgorithm extends BaseDualProbQueryAlgorithm[Element] {
...
trait DualStreamableProbQueryAlgorithm extends DualProbQueryAlgorithm {
  /**
    * Sample an value from the posterior of this element
    */
  def sampleFromPosterior[T](element: Element[T]): Stream[T]
