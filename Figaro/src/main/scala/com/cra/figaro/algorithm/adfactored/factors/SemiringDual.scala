...
case class SumProductDualSemiring() extends DivideableSemiRing[(Double, Double)] {
  /**
    ...
    */
  def product(x: (Double, Double), y: (Double, Double)) = (x._1 * y._1, x._1 * y._2 + x._2 * y._1)

  def product(x: (Double), y: (Double, Double)) = (x * y._1, x * y._2 + 0 * y._1)

  def product(x: (Double, Double), y: (Double)): (Double, Double) = { product(y, x) }


  /**
    * Dividing with dual numbers.
    */
  def divide(x: (Double, Double), y: (Double, Double)) = (x._1 / y._1,  x._2 * y._1 - x._1 * y._2 / (y._1 * y._1))

  def divide(x: (Double), y: (Double, Double)) = (x / y._1,  0 * y._1 - x * y._2 / (y._1 * y._1))

  def divide(x: (Double, Double), y: (Double)) = (x._1 / y,  x._2 * y - x._1 * 0 / (y * y))

  /**
    * Decision joint factor marginalization.
    */
  def sum(x: (Double, Double), y: (Double, Double)) = (x._1 + y._1 , x._2 + y._2)

  def sum(x: (Double), y: (Double, Double)) = (x + y._1 , 0 + y._2)

  def sum(x: (Double, Double), y: (Double)): (Double, Double) = { sum(y,x) }
