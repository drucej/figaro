...
class ADVariableEliminationTest extends WordSpec with Matchers {
  "ADVE" should {
    ...
    "get the same answers as Dan's example" in {
      def makeModel(param: Double) = {
        ...
        (pIndGivenThreat, indicator)
      }

      def modelGen(param: Double) = {
        val (derivativeTarget, inferenceTarget) = makeModel(param)
        inferenceTarget
      }

      val initialParameterValue = 0.9
      val targetValue = true

      val expectedDerivative = computeDerivativeNumerically(initialParameterValue, modelGen, targetValue)
      println(s"Expected derivative: $expectedDerivative")
      val (derivativeTarget, inferenceTarget) = makeModel(initialParameterValue)
      val (prob, deriv) = runADVEandVE(derivativeTarget, inferenceTarget, targetValue)
    ...
    "(fails) get the same answers as Dan's example with a no-op Select" in {
      val pThreat =  Constant(0.25)
      val pIndGivenThreat = Constant(0.9)
      val pIndGivenNotThreat = Constant(0.15)
      val threat = Flip(pThreat)
    ...
    "(fails) get answers in Dan's example with a squared value in the Apply" in {
      def makeModel(param: Double) = {
        Universe.createNew() // it's critical that we clear the universe after computing expected derivative
        val pThreat =  Constant(0.25)
        val pIndGivenThreat = Constant(param)
    ...
    // this one should work
    "work on Jeff's Model3Node with a Constraint" in {
      def makeModel(param: Double) = {
        Universe.createNew() // it's critical that we clear the universe after computing expected derivative
        val pThreat = 0.25
        val pIndGivenThreat = Constant(0.7)
        val pIndGivenNotThreat = Constant(0.15)
        val pAlertGivenInd = Constant(param)
        val pAlertGivenNotInd = Constant(0.12)
        val Threat = Flip(pThreat)
        val Ind = Flip(If(Threat, pIndGivenThreat, pIndGivenNotThreat))
        val Alert = Flip(If(Ind, pAlertGivenInd, pAlertGivenNotInd))
    ...
    "work with a beta in a modified Jeff's Model3Node" in {
      ...
      val initialParameterValue = 0.25
      val targetValue = true

      def seedHack() =  com.cra.figaro.util.setSeed(44545)
      val expectedDerivative = computeDerivativeNumerically(initialParameterValue, modelGen, targetValue, seedHack)
      println(s"Expected derivative: $expectedDerivative")
      val (derivativeTarget, inferenceTarget) = makeModel(initialParameterValue)
      seedHack()
      val (prob, deriv) = runADVEandVE(derivativeTarget, inferenceTarget, targetValue)
    ...
    "work on Jeff's Model3IndicatorDownSelect" in {
      ...
      val expectedDerivative = computeDerivativeNumerically(0.9, modelGen, true)
      println(s"Expected derivative: $expectedDerivative")
  ...
  def checkValue(result: Double, ideal: Double) = {
    val tolerance = 5e-2
    println(f"Difference from ideal: ${result - ideal}")
